...
public class DatabaseSeeder
{
    ...
    {
        _logger.LogDebug("Seeding attendees...");
        
        string? timetablesSeed = Environment.GetEnvironmentVariable("SEED_ATTENDEES");
        ...
        foreach (var slot in slots.EnumerateArray())
        {
            var studentId = slot.GetProperty("studentId").GetInt32();
    ...
    {
        _logger.LogDebug("Seeding semester...");
        
        string? semesterNameSeed = Environment.GetEnvironmentVariable("SEED_SEMESTER_NAME");
        ArgumentException.ThrowIfNullOrWhiteSpace(semesterNameSeed, "SEED_SEMESTER_NAME");
        
        string? semesterStartSeed = Environment.GetEnvironmentVariable("SEED_SEMESTER_START");
        ArgumentException.ThrowIfNullOrWhiteSpace(semesterStartSeed, "SEED_SEMESTER_START");
        
        string? semesterEndSeed = Environment.GetEnvironmentVariable("SEED_SEMESTER_END");
        ArgumentException.ThrowIfNullOrWhiteSpace(semesterEndSeed, "SEED_SEMESTER_END");
        
        var semesterStart = DateTimeOffset.FromUnixTimeSeconds(int.Parse(semesterStartSeed));
        var semesterEnd = DateTimeOffset.FromUnixTimeSeconds(int.Parse(semesterEndSeed));
        
        if (!await _context.Semesters.AnyAsync())
        {
            var semester = new SemesterModel
            {
                Name = semesterNameSeed,
                StartDate = DateOnly.FromDateTime(semesterStart.UtcDateTime),
                EndDate = DateOnly.FromDateTime(semesterEnd.UtcDateTime),
