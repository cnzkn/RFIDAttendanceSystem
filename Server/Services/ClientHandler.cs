...
public class ClientHandler : IClientHandler
{
    ...
    private async Task SendInitialListAsync(Guid connectionId, Guid sessionId)
    {
        ...
        _logger.LogInformation($"[ClientHandler] Found {timetable.CourseSection.Attendees.Count} attendees for session {sessionId}. Logs found: {logs.Count} (Week {currentWeek})");
        ...
        if (_sockets.TryGetValue(connectionId, out var socket) && socket.State == WebSocketState.Open)
        {
            ...
            _logger.LogInformation($"[ClientHandler] Sent initial_list to {connectionId}. Payload size: {bytes.Length} bytes");
        }
        else 
        {
            _logger.LogWarning($"[ClientHandler] Could not send initial_list. Socket not found or closed. ID: {connectionId}");
        }
    }
